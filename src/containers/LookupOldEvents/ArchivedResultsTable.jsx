...
class ArchivedResultsTable extends React.Component {
  ...
  render() {
    const { researchersEmailById } = this.props;
    const components = {
      body: {
        row: EditableFormRow,
        cell: EditableCell,
    ...
    let columns = [
      ...
      },
      {
        ...
        render: (text, record) => {
          ...
          if (record.district) {
            return `${record.state}-${
              record.district === "At-Large" ? "00" : record.district
            }`;
          }
          return record.state;
        },
...
function mapStateToProps(state) {
  return {
    ...
    researchersEmailById: researcherStateBranch.selectors.getResearchersEmailById(
      state
    ),
  };
}
...
const mapDispatchToProps = (dispatch) => ({
  validateAndSaveOldEvent: (data) =>
    dispatch(eventsStateBranch.actions.validateAndSaveOldEvent(data)),
});
