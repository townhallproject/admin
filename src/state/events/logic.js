...
import { requestResearcherById } from "../researchers/actions";
import {
  getDateLookupType,
...
const fetchFederalAndStateLiveEvents = createLogic({
  ...
  process(deps) {
    ...
    const getEventsPromises = [
      firebasedb.ref("state_townhalls").once("value"),
...
const fetchOldEventsLogic = createLogic({
  ...
  process({ getState, action, firestore }, dispatch, done) {
    const { payload } = action;
    ...
    if (dateKey === DATE_CREATED) {
      fsQueryRef = fsRef.orderBy("timestamp");
    } else {
      fsQueryRef = fsRef
...
const approveEventLogic = createLogic({
  ...
  process(deps, dispatch, done) {
    ...
    const { townHall, path, livePath } = action.payload;
    ...
    dispatch(setLoading(true));
    firebasedb
      .ref(`${livePath}/${townHall.eventId}`)
      .update(cleanTownHall)
      .then(() => {
        const approvedTownHall = firebasedb.ref(
          `${path}/${cleanTownHall.eventId}`
        );
        approvedTownHall.remove().then(() => {
          townHallMetaData
            .update({
              status: "live",
            })
...
const archiveEventLogic = createLogic({
  type: ARCHIVE_EVENT,
  process(deps, dispatch, done) {
    const { action, firebasedb, getState } = deps;

    const { townHall, path } = action.payload;
    const level =
      getActiveFederalOrState(getState()) === FEDERAL_RADIO_BUTTON
        ? FEDERAL_RADIO_BUTTON
        : "state";
    const cleanTownHall = {
      ...townHall,
    ...
    const oldTownHallID = firebasedb.ref(
      `/townHallIds/${cleanTownHall.eventId}`
    );
    dispatch(setLoading(true));
    const url =
...
const requestTotalEventsCounts = createLogic({
  ...
  process(deps, dispatch, done) {
    const { firebasedb } = deps;
    firebasedb
      .ref(`${EVENTS_PATHS[PENDING_EVENTS_TAB].STATE}`)
      .on("value", (snapshot) => {
    ...
    firebasedb
      .ref(`${EVENTS_PATHS[PENDING_EVENTS_TAB].FEDERAL}`)
      .on("value", (snapshot) => {
        dispatch(
          requestFederalTotalEventsCountsSuccess(snapshot.numChildren())
        );
      });
  },
});
